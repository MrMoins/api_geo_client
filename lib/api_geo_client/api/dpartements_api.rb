...
module ApiGeoClient
  class DpartementsApi
    ...
    # @param [Hash] opts the optional parameters
    # @option opts [Array<String>] :fields Liste des champs souhaités dans la réponse
    # @return [Array<(Departement, Integer, Hash)>] Departement data, response status code and response headers
    def departements_code_get_with_http_info(code, opts = {})
      ...
      if @api_client.config.client_side_validation && opts[:'fields'] && !opts[:'fields'].all? { |item| ['nom', 'code', 'codeRegion', 'region', 'zone'].include?(item) }
        fail ArgumentError, 'invalid value for "fields", must include one of nom, code, codeRegion, region, zone'
      end
      # resource path
      ...
      return_type = opts[:return_type] || 'Departement' 
